...
public class SysClientServiceImpl implements ISysClientService {
    ...
    public TableDataInfo<SysClientVo> queryPageList(SysClientBo bo, PageQuery pageQuery) {
        ...
        result.getRecords().forEach(r -> {
            r.setGrantTypeList(StringUtils.splitList(r.getGrantType()));
            r.setMiniAppSecret(null);
        });
    ...
    private LambdaQueryWrapper<SysClient> buildQueryWrapper(SysClientBo bo) {
        ...
        lqw.eq(StringUtils.isNotBlank(bo.getMiniAppId()), SysClient::getMiniAppId, bo.getMiniAppId());
    ...
    private void validateMiniAppConfig(SysClientBo client) {
        boolean miniAppClient = "xcx".equals(client.getDeviceType())
            || CollUtil.contains(client.getGrantTypeList(), "xcx");
        if (miniAppClient && (StringUtils.isBlank(client.getMiniAppId())
            || StringUtils.isBlank(client.getMiniAppSecret()))) {
            throw new ServiceException("小程序客户端必须填写微信小程序 AppID 和 AppSecret");
        }
    }
