...
import (
	...
	"github.com/prometheus/blackbox_exporter/pingagent"
...
var (
	...
	pingAgentEnabled       = kingpin.Flag("ping-agent.enabled", "Run the okping node Agent alongside blackbox_exporter.").Envar("PING_AGENT_ENABLED").Default("false").Bool()
	pingAgentMasterURL     = kingpin.Flag("ping-agent.master-url", "okping controller base URL.").Envar("PING_AGENT_MASTER_URL").Default("http://127.0.0.1:8080").String()
	pingAgentUUID          = kingpin.Flag("ping-agent.uuid", "UUID configured for this node in the okping controller.").Envar("PING_AGENT_UUID").String()
	pingAgentUUIDFile      = kingpin.Flag("ping-agent.uuid-file", "File containing the UUID configured for this node.").Envar("PING_AGENT_UUID_FILE").Default("/etc/ping-agent/uuid").String()
	pingAgentRetryInterval = kingpin.Flag("ping-agent.retry-interval", "Delay before retrying failed Agent registration or RabbitMQ setup.").Envar("PING_AGENT_RETRY_INTERVAL").Default("30s").Duration()
...
func run() int {
	...
	defer cancelAgent()
	if *pingAgentEnabled {
		uuid, err := loadPingAgentUUID(*pingAgentUUID, *pingAgentUUIDFile)
		if err != nil {
			logger.Error("Invalid ping Agent configuration", "err", err)
			return 1
		}
		runner, err := pingagent.New(pingagent.Config{
			MasterURL:     *pingAgentMasterURL,
			UUID:          uuid,
			Version:       version.Version,
			RetryInterval: *pingAgentRetryInterval,
		}, logger)
		if err != nil {
			logger.Error("Invalid ping Agent configuration", "err", err)
			return 1
		...
		go func() {
			if err := runner.Run(agentContext); err != nil && !errors.Is(err, context.Canceled) {
				logger.Error("ping Agent stopped", "err", err)
			}
		}()
		logger.Info("Started ping Agent", "uuid", uuid, "master", *pingAgentMasterURL)
	}
...
func loadPingAgentUUID(value string, fileName string) (string, error) {
	if value = strings.TrimSpace(value); value != "" {
		return value, nil
	}
	data, err := os.ReadFile(fileName)
	if err != nil {
		return "", fmt.Errorf("read ping Agent UUID file %s: %w", fileName, err)
	}
	uuid := strings.TrimSpace(string(data))
	if uuid == "" {
		return "", errors.New("ping Agent UUID is empty")
	}
	return uuid, nil
}
