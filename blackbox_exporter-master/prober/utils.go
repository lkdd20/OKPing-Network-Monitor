...
func chooseProtocol(ctx context.Context, IPProtocol string, fallbackIPProtocol bool, target string, registry *prometheus.Registry, logger *slog.Logger) (ip *net.IPAddr, lookupTime float64, err error) {
	...
	probeIPAddrV4 := prometheus.NewGauge(prometheus.GaugeOpts{
		Name: "probe_ip_addr_v4",
		Help: "Specifies the resolved IPv4 address as an integer. Returns 0 for non-IPv4 addresses.",
	})
	registry.MustRegister(probeIPProtocolGauge)
	registry.MustRegister(probeDNSLookupTimeSeconds)
	registry.MustRegister(probeIPAddrHash)
	registry.MustRegister(probeIPAddrV4)
	...
	if !fallbackIPProtocol {
		...
		if err == nil {
			for _, ip := range ips {
				...
				probeIPAddrV4.Set(ipV4ToInt(ip))
	...
	for _, ip := range ips {
		...
		case "ip4":
			if ip.IP.To4() != nil {
				...
				probeIPAddrV4.Set(ipV4ToInt(ip.IP))
	...
	probeIPAddrV4.Set(ipV4ToInt(fallback.IP))
...
func ipV4ToInt(ip net.IP) float64 {
	v4 := ip.To4()
	if v4 == nil {
		return 0
	}
	return float64(binary.BigEndian.Uint32(v4))
}
