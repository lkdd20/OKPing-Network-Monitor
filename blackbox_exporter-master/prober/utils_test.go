...
func TestChooseProtocolSetsIPv4IntegerMetric(t *testing.T) {
	ctx := context.Background()
	registry := prometheus.NewPedanticRegistry()
	logger := promslog.NewNopLogger()

	ip, _, err := chooseProtocol(ctx, "ip4", false, "127.0.0.1", registry, logger)
	if err != nil {
		t.Fatal(err)
	}
	if ip == nil || !ip.IP.Equal(net.ParseIP("127.0.0.1")) {
...
func TestIPV4ToInt(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.ip, func(t *testing.T) {
			if got := ipV4ToInt(net.ParseIP(tt.ip)); got != tt.expected {
