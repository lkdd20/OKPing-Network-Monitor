...
import { fetchIPv6ScreenNodes } from "@/lib/ping/server";
...
export async function POST(request: Request) {
  ...
  try {
    return Response.json({ code: 200, data: await fetchIPv6ScreenNodes(query) });
  } catch (error) {
    const message = error instanceof Error && error.message ? error.message : "IPv6 筛选节点请求失败";
