...
import { ensureProbeTargetPort } from "@/lib/ping/target";
import type { ProbeModel } from "@/lib/ping/types";

type TcpingRouteContext = {
  params: Promise<{
    target?: string[];
  }>;
};

type TcpingRequestBody = {
  dns?: unknown;
  model?: unknown;
  operators?: unknown;
  region?: unknown;
...
async function getRequestBody(request: Request) {
  ...
  return (await request.json().catch(() => null)) as TcpingRequestBody | null;
}
...
function getUrlFromBody(body: TcpingRequestBody | null) {
...
export async function POST(request: Request, { params }: TcpingRouteContext) {
  await params;
  ...
  try {
    const payloadTarget = ensureProbeTargetPort(url, 80);
