...
function getCanonicalPath(initialUrl: string) {
  return buildProbeSharePath("/http", initialUrl);
}
...
function fallbackPageConfig(initialUrl: string): PingPublicPageConfig {
  ...
  return {
    ...
    description: `Ping 为 ${displayTarget} 提供未登录用户网站测速，支持多地区、多运营商节点实时返回响应 IP、状态码、请求头与响应时间。`,
    h1: `${displayTarget} 网站测速`,
    intro: `当前检测目标：${displayTarget}。页面参数由服务端渲染输出，便于搜索引擎解析当前检测内容。`,
    keywords: `Ping,网站测速,HTTP测速,${initialUrl},WEB测试,CDN测速`,
    pageKey: "http",
    pageName: "网站测速",
    target: initialUrl,
    title: `${displayTarget} 网站测速 - Ping`,
  };
}
...
async function getPageConfig(initialUrl: string) {
  ...
  try {
    pageConfig = await fetchPublicPageConfig("http", initialUrl, getCanonicalPath(initialUrl));
  } catch {
...
export async function generateMetadata({ params, searchParams }: HTTPPageProps): Promise<Metadata> {
...
export default async function HTTPPage({ params, searchParams }: HTTPPageProps) {
  const { target } = await params;
  const query = await searchParams;
  const initialUrl = getStringParam(query.url)?.trim() || probeTargetFromPathSegments(target);
  ...
  return (
    <PingClient
      ...
      placeholder="请输入网址，例如：example.com、https://example.com/index.html"
      probeLabel="网站测速"
      probeType="http"
      secondaryActionLabel="缓慢测试"
      secondaryActionModel="slow"
      screenshotSuffix="http"
    />
  );
}
