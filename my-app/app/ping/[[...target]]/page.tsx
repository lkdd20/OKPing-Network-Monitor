...
function getCanonicalPath(initialUrl: string) {
  return buildProbeSharePath("/ping", initialUrl);
}
...
function fallbackPageConfig(initialUrl: string): PingPublicPageConfig {
  ...
  return {
    ...
    description: `Ping 为 ${displayTarget} 提供未登录用户在线 Ping 检测，支持多地区、多运营商节点实时返回响应 IP、地区与延迟。`,
    h1: `${displayTarget} 在线 Ping 检测`,
    intro: `当前检测目标：${displayTarget}。页面参数由服务端渲染输出，便于搜索引擎解析当前检测内容。`,
    keywords: `Ping,在线Ping,${initialUrl},网络检测,延迟检测`,
    pageKey: "ping",
    pageName: "在线 Ping",
    target: initialUrl,
    title: `${displayTarget} 在线 Ping 检测 - Ping`,
  };
}
...
async function getPageConfig(initialUrl: string) {
  ...
  try {
    pageConfig = await fetchPublicPageConfig("ping", initialUrl, getCanonicalPath(initialUrl));
  } catch {
...
export async function generateMetadata({ params, searchParams }: PingPageProps): Promise<Metadata> {
...
export default async function PingPage({ params, searchParams }: PingPageProps) {
  const { target } = await params;
  const query = await searchParams;
  const initialUrl = getStringParam(query.url)?.trim() || probeTargetFromPathSegments(target);
