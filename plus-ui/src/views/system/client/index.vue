  ...
  <div class="p-2">
    ...
    <el-card shadow="never">
      ...
      <el-table v-loading="loading" :data="clientList" border @selection-change="handleSelectionChange">
        ...
        <el-table-column label="小程序 AppID" align="center" prop="miniAppId">
          <template #default="scope">{{ scope.row.miniAppId || '-' }}</template>
        </el-table-column>
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.title" width="500px" append-to-body>
      <el-form ref="clientFormRef" :model="form" :rules="rules" label-width="100px">
        ...
        <template v-if="isMiniAppClient">
          ...
          <el-form-item label="小程序码版本" prop="miniAppEnvVersion">
            <el-select v-model="form.miniAppEnvVersion" placeholder="请选择小程序码版本">
...
const isMiniAppClient = computed(() => form.value.deviceType === 'xcx' || form.value.grantTypeList?.includes('xcx'));
...
const submitForm = () => {
  clientFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      if (isMiniAppClient.value && (!form.value.miniAppId?.trim() || !form.value.miniAppSecret?.trim())) {
        proxy?.$modal.msgError('小程序客户端必须填写微信小程序 AppID 和 AppSecret');
        return;
      }
